...
impl LastVotedForkSlotsAggregate {
    ...
    pub(crate) fn new(
        ...
        for slot in my_last_voted_fork_slots {
            if slot >= &root_slot {
                ...
                if let Some(sender_stake) = Some(Self::validator_stake(root_bank.epoch_stakes(epoch).unwrap(), my_pubkey)) {
        ...
        let epoch_info_vec: Vec<LastVotedForkSlotsEpochInfo> = (root_epoch
            ...
            .map(|epoch| {
                ...
                let my_stake = Self::validator_stake(root_bank.epoch_stakes(epoch).unwrap(), my_pubkey);
    ...
    fn insert_message(&mut self, from: &Pubkey, new_slots_vec: &Vec<Slot>) {
        for slot in new_slots_vec {
            ...
            if let Some(sender_stake) = Some(Self::validator_stake(self.root_bank.epoch_stakes(epoch).unwrap(), from)) {
    ...
    fn update_epoch_info(&mut self, from: &Pubkey, last_vote_epoch: Epoch) {
        for entry in self.epoch_info_vec.iter_mut() {
            if let Some(stake) = Some(Self::validator_stake(self.root_bank.epoch_stakes(entry.epoch).unwrap(), from)) {
